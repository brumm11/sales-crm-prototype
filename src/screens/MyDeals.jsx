...
import { AlertTriangle, Pause, FileClock, List, Inbox } from '../components/Icons';
...
export default function MyDeals() {
  ...
  const filtered = useMemo(() => {
    switch (active) {
      case 'attention':
        return deals.filter((d) => d.priority.needsAttention);
      case 'hold':
        return deals.filter((d) => d.priority.onHold);
      case 'docs':
        return deals.filter((d) => d.priority.hasPendingDocs);
      default:
        return deals;
    }
  }, [deals, active]);
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="no-scrollbar sticky top-14 z-20 flex gap-2 overflow-x-auto border-b border-neutral-100 bg-white/85 px-5 py-3 backdrop-blur-md">
        {FILTERS.map((f) => {
